...
public class UlsTracker {
    ...
    public UlsTracker(final Context context) {
        boolean ok = naInitialiseFromAssets(context, context.getAssets(), context.getCacheDir().getAbsolutePath());
        if (ok) {
           createVariables();
        } else {
    ...
    public UlsTracker(final Context context, final String path) {
        boolean ok = naInitialiseFromPath(context, path);
        if (ok) {
           createVariables();
        } else {
...
//endregion
    ...
    private native boolean naInitialiseFromAssets(Context context, AssetManager manager, String cacheDir);
    private native boolean naInitialiseFromPath(Context context, String path);
