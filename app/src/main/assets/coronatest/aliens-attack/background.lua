...
local function loadCity()
    ...
    local c = display.newImageRect("assets/images/city.png", screenW, h)
    c.anchorX = 0
    c.anchorY = 0
    c.y = screenH * 0.85 - h
...
local function popSmallCloud()
    ...
    cloud.x, cloud.y = -cloud.width, math.random(0, 100)
    cloud.alpha = math.random()

    local removeCloud = function() display.remove(cloud) end
    transition.to(cloud, { time = 5000, x = screenW + cloud.width, onComplete = removeCloud })
end
...
local function popCloud()
    ...
    cloud.x, cloud.y = -cloud.width, math.random(-100, 50)
    cloud.alpha = math.random()
