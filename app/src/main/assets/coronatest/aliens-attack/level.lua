...
local function startGame(event)
    ...
    log('startGame - before delay')



    -- function listener()
    --     log('startGame - after delay')
    --  messenger.broadcastUserReady()
    --
    --  if isMaster() then
    --
    --      log('startGame - I AM master')
    --      resetScores()
    ...
    --  log(time .. " seconds later")
    -- end

    -- log("now")
    -- timer.performWithDelay(5000, function() when(5) end)
    -- timer.performWithDelay(10000, function() when(10) end)
...
local function useWatch()
    ...
    paceFactorTimer = timer.performWithDelay(5000, listener())
end
...
local function createAlien(occurrence)
    ...
    messenger.broadcastPopAlien(alien, 1)

    function listener()
        createAlien(occurrence + 1)
    end

    createAlienTimer = timer.performWithDelay(level.popInterval() * 1000, listener())
end
...
local function newGame(fromUserId, timestamp, playersIds)
    ...
    if isMaster() then
        log('newGame - I AM master')
        log('newGame - before delay')
        function listener()
            log('newGame - after delay')
            createAlien(0)
        end

        timer.performWithDelay(getDelayUntilTimestamp(timestamp), listener())
    else
        log('newGame - I AM NOT master')
    end
end
...
function scene:show(event)
    ...
    if event.phase == "did" then
        ...
        if isSimulator then
            ...
            startGame({ myUserId = user.id, masterUserId = user.id, playersUsers = { user } })
        end
    end
end
