...
function NewStick( Props )
		...
		-- for use images uncomment the line down and comment line up
		Group.Thumb = display.newImageRect(Props.joystickImage, Props.thumbSize, Props.thumbSize)
        ...
        Group.Overlay = display.newImageRect(Props.joystickOverlay, 60, 60)
        Group.Overlay.y = -2
        Group:insert(Group.Overlay)
        ...
        function Group:move(Obj, maxSpeed, rotate)
            ...
            local deltaX = Cos( Rad(self.angle-90) ) * (maxSpeed * self.percent) 
            local deltaY = Sin( Rad(self.angle-90) ) * (maxSpeed * self.percent)
        ...
        function Group:getAngleRad() return Rad(Ceil(self.angle)) end
