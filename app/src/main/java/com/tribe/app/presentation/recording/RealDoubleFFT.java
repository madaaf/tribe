...
public class RealDoubleFFT extends RealDoubleFFT_Mixed {
   ...
   * the (<em>n</em>-<em>k</em>)-th complex FFT coefficient is the conjugate of <em>n</em>-th
  ...
  /**
   * Backward real FFT transform. It is the unnormalized inverse transform of <em>ft</em>(Complex1D,
   * double[]).
   *
   * @param x an array which contains the sequence to be transformed. When <em>n</em> is odd, it
   * contains the first
   * (<em>n</em>+1)/2 complex data; when <em>n</em> is even, it contains (<em>n</em>/2+1) complex
   * data.
