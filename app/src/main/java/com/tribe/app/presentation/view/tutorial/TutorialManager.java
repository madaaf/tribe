...
@Singleton public class TutorialManager {

  private ScreenUtils screenUtils;
  private Preference<Set<String>> tutorialState;

  @StringDef({ MESSAGES_SUPPORT, NEXT, CLOSE, REPLY_MODE, REPLY, RELEASE, CANCEL })
  ...
  @Inject public TutorialManager(ScreenUtils screenUtils,
  ...
  public boolean shouldDisplay(@TutorialKey String key) {
    ...
    if (tutorialState.get().contains(MESSAGES_SUPPORT)
        || key.equals(CLOSE) && tutorialState.get()
        .contains(NEXT)
  ...
  public Tutorial showMessagesSupport(Activity activity, View target, int toolTipOffsetX,
      int toolTipOffsetY, int holeRadius, int holeRadiusPulsePadding, Bitmap overlayImage,
      int overlayImageSize) {
    if (activity != null) {
      return Tutorial.init(activity, screenUtils, MESSAGES_SUPPORT)
          .with(Tutorial.CLICK)
          .setToolTip(new ToolTip(activity, screenUtils).setTitle(
              activity.getString(R.string.tutorial_message_support, 2))
              .setGravity(Gravity.BOTTOM | Gravity.LEFT)
              .setOffsetX(toolTipOffsetX)
              .setOffsetY(toolTipOffsetY)
              .setBackgroundRes(R.drawable.bg_tuto_right_upward))
          .setOverlay(new Overlay(activity).setStyle(Overlay.CIRCLE)
  ...
  public Tutorial showNext(Activity activity, View target, int toolTipOffsetX, int toolTipOffsetY,
    ...
    if (activity != null) {
      return Tutorial.init(activity, screenUtils, NEXT)
          .with(Tutorial.CLICK)
          .setToolTip(new ToolTip(activity, screenUtils).setTitle(
              activity.getString(R.string.tutorial_message_next_message))
  ...
  public Tutorial showClose(Activity activity, View target, int toolTipOffsetX, int toolTipOffsetY,
      int holeRadius, int holeRadiusPulsePadding, Bitmap overlayImage, int overlayImageSize) {
    if (activity != null) {
      return Tutorial.init(activity, screenUtils, CLOSE)
          .with(Tutorial.CLICK)
          .setToolTip(new ToolTip(activity, screenUtils).setTitle(
              activity.getString(R.string.tutorial_message_close_message))
              .setGravity(Gravity.TOP | Gravity.LEFT)
              .setOffsetX(toolTipOffsetX)
              .setOffsetY(toolTipOffsetY)
              .setBackgroundRes(R.drawable.bg_tuto_right_downward))
  ...
  public Tutorial showReplyMode(Activity activity, View target, int holeRadius,
      int holeRadiusPulsePadding, View.OnClickListener onClickListener) {
    if (activity != null) {
      return Tutorial.init(activity, screenUtils, REPLY_MODE)
          .with(Tutorial.CLICK)
          .setToolTip(new ToolTip(activity, screenUtils).setTitle(
              activity.getString(R.string.tutorial_message_reply))
  ...
  public Tutorial showReply(Activity activity, View target, int holeRadius,
      int holeRadiusPulsePadding, View.OnClickListener onClickListener) {
    if (activity != null) {
      return Tutorial.init(activity, screenUtils, REPLY)
          .with(Tutorial.CLICK)
          .setToolTip(new ToolTip(activity, screenUtils).setTitle(
              activity.getString(R.string.tutorial_message_record_reply))
              .setGravity(Gravity.TOP | Gravity.CENTER_HORIZONTAL)
              .setBackgroundRes(R.drawable.bg_tuto_center_downward))
          .setOverlay(new Overlay(activity).setStyle(Overlay.CIRCLE)
  ...
  public Tutorial showRelease(Activity activity, View target) {
    if (activity != null) {
      return Tutorial.init(activity, screenUtils, RELEASE)
          .with(Tutorial.CLICK)
          .setToolTip(new ToolTip(activity, screenUtils).setTitle(
              activity.getString(R.string.tutorial_message_release))
              .setGravity(Gravity.TOP | Gravity.CENTER_HORIZONTAL)
  ...
  public Tutorial showTapToCancel(Activity activity, View target, int holeRadius,
      int holeRadiusPulsePadding, View.OnClickListener onClickListener) {
    if (activity != null) {
      return Tutorial.init(activity, screenUtils, CANCEL)
          .with(Tutorial.CLICK)
          .setToolTip(new ToolTip(activity, screenUtils).setTitle(
              activity.getString(R.string.tutorial_message_tap_to_cancel))
              .setGravity(Gravity.TOP | Gravity.CENTER_HORIZONTAL)
              .setBackgroundRes(R.drawable.bg_tuto_center_downward))
          .setOverlay(new Overlay(activity).setStyle(Overlay.CIRCLE)
              .setHoleRadius(holeRadius)
