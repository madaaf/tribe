...
public class Tutorial {
  ...
  @IntDef({ ALLOW_ALL, CLICK_ONLY, SWIPE_ONLY }) public @interface MotionType {
  }
  ...
  public static Tutorial init(Activity activity, ScreenUtils screenUtils,
      @TutorialManager.TutorialKey String key) {
  ...
  private void setupToolTip() {
    ...
    if (toolTip != null) {
      ...
      toolTipViewGroup.measure(FrameLayout.LayoutParams.WRAP_CONTENT,
          FrameLayout.LayoutParams.WRAP_CONTENT);
      ...
      final float adjustment =
          10 * density; //adjustment is that little overlapping area of tooltip and targeted button
      ...
      if (toolTipMeasuredWidth > parent.getWidth()) {
        resultPoint.x = getXForTooTip(toolTip.gravity, parent.getWidth(), targetViewX,
            adjustment + toolTip.offsetX);
      } else {
        resultPoint.x = getXForTooTip(toolTip.gravity, toolTipMeasuredWidth, targetViewX,
            adjustment + toolTip.offsetX);
      }

      resultPoint.y = getYForTooTip(toolTip.gravity, toolTipMeasuredHeight, targetViewY,
          adjustment + toolTip.offsetY);
      ...
      if (resultPoint.x < 0) {
        toolTipViewGroup.getLayoutParams().width =
      ...
      if (tempRightX > parent.getWidth()) {
        toolTipViewGroup.getLayoutParams().width =
            parent.getWidth() - resultPoint.x; //since point.x is negative, use plus
      }
      ...
      toolTipViewGroup.getViewTreeObserver()
          .addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            @Override public void onGlobalLayout() {
              ...
              fixedY = getYForTooTip(toolTip.gravity, toolTipHeightAfterLayouted, targetViewY,
                  adjustment);
  ...
  private int getXForTooTip(int gravity, int toolTipMeasuredWidth, int targetViewX,
      float adjustment) {
    int x;
  ...
  private int getYForTooTip(int gravity, int toolTipMeasuredHeight, int targetViewY,
      float adjustment) {
    int y;

    if ((gravity & Gravity.TOP) == Gravity.TOP) {

      if (((gravity & Gravity.LEFT) == Gravity.LEFT) || ((gravity & Gravity.RIGHT)
          == Gravity.RIGHT)) {
