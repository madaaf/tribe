package com.tribe.app.presentation.view.player;
        ...
        implements MediaPlayer.EventListener
{
    ...
    protected MediaPlayerWrapper(MediaPlayer mediaPlayer) {
        TAG = "" + this;
        if (SHOW_LOGS) Log.v(TAG, "constructor of MediaPlayerWrapper");
        if (SHOW_LOGS) Log.v(TAG, "constructor of MediaPlayerWrapper, main Looper " + Looper.getMainLooper());
        if (SHOW_LOGS) Log.v(TAG, "constructor of MediaPlayerWrapper, my Looper " + Looper.myLooper());
...
//      TODO: if (SHOW_LOGS) Log.err(TAG, "catch exception, is Network Connected [" + Utils.isNetworkConnected());
    ...
    public void setDataSource(String filePath) throws IOException {
        synchronized (mState) {
            if (SHOW_LOGS) Log.v(TAG, "setDataSource, filePath " + filePath + ", mState " + mState);
    ...
    public void onCompletion(MediaPlayer mp) {
        if (SHOW_LOGS) Log.v(TAG, "onVideoCompletion, mState " + mState);
    ...
    public boolean onError(MediaPlayer mp, int what, int extra) {
        if (SHOW_LOGS) Log.v(TAG, "onErrorMainThread, what " + what + ", extra " + extra);
        ...
        if (SHOW_LOGS) Log.v(TAG, "onErrorMainThread, mListener " + mListener);
    ...
    private void printInfo(int what) {
        switch (what) {
            case MediaPlayer.MEDIA_INFO_UNKNOWN:
                if (SHOW_LOGS) Log.i(TAG, "onInfo, MEDIA_INFO_UNKNOWN");
                break;
            case MediaPlayer.MEDIA_INFO_VIDEO_TRACK_LAGGING:
                if (SHOW_LOGS) Log.i(TAG, "onInfo, MEDIA_INFO_VIDEO_TRACK_LAGGING");
                break;
            case MediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START:
                if (SHOW_LOGS) Log.i(TAG, "onInfo, MEDIA_INFO_VIDEO_RENDERING_START");
                break;
            case MediaPlayer.MEDIA_INFO_BUFFERING_START:
                if (SHOW_LOGS) Log.i(TAG, "onInfo, MEDIA_INFO_BUFFERING_START");
                break;
            case MediaPlayer.MEDIA_INFO_BUFFERING_END:
                if (SHOW_LOGS) Log.i(TAG, "onInfo, MEDIA_INFO_BUFFERING_END");
                break;
            case MediaPlayer.MEDIA_INFO_BAD_INTERLEAVING:
                if (SHOW_LOGS) Log.i(TAG, "onInfo, MEDIA_INFO_BAD_INTERLEAVING");
                break;
            case MediaPlayer.MEDIA_INFO_NOT_SEEKABLE:
                if (SHOW_LOGS) Log.i(TAG, "onInfo, MEDIA_INFO_NOT_SEEKABLE");
                break;
            case MediaPlayer.MEDIA_INFO_METADATA_UPDATE:
                if (SHOW_LOGS) Log.i(TAG, "onInfo, MEDIA_INFO_METADATA_UPDATE");
                break;
            case MediaPlayer.MEDIA_INFO_UNSUPPORTED_SUBTITLE:
                if (SHOW_LOGS) Log.i(TAG, "onInfo, MEDIA_INFO_UNSUPPORTED_SUBTITLE");
                break;
            case MediaPlayer.MEDIA_INFO_SUBTITLE_TIMED_OUT:
                if (SHOW_LOGS) Log.i(TAG, "onInfo, MEDIA_INFO_SUBTITLE_TIMED_OUT");
                break;
        }
    }
     ...
     */
    public void start() {
        if (SHOW_LOGS) Log.v(TAG, ">> start");

        synchronized (mState) {
            if (SHOW_LOGS) Log.v(TAG, "start, mState " + mState);
            ...
            switch (mState.get()) {
                ...
                case PAUSED:
                    ...
                    if (SHOW_LOGS) Log.v(TAG, "start, video is " + mState + ", starting playback.");
                    mMediaPlayer.start();
     ...
     */
    public void pause() {
        if (SHOW_LOGS) Log.v(TAG, ">> pause");

        synchronized (mState) {
            if (SHOW_LOGS)
                Log.v(TAG, "pause, mState " + mState);
    ...
    private final Runnable mOnVideoStopMessage = new Runnable() {
        @Override
        public void run() {
            if (SHOW_LOGS) Log.v(TAG, ">> run, onVideoStoppedMainThread");
    ...
    public void clearAll() {
        if (SHOW_LOGS) Log.v(TAG, ">> clearAll, mState " + mState);
        synchronized (mState) {
    ...
    public void setSurfaceTexture(SurfaceTexture surfaceTexture) {
        ...
        if (SHOW_LOGS) Log.v(TAG, "setSurfaceTexture mSurface " + mSurface);
    ...
    private void startPositionUpdateNotifier() {
        if (SHOW_LOGS)
            Log.v(TAG, "startPositionUpdateNotifier, mPositionUpdateNotifier " + mPositionUpdateNotifier);
    ...
    private void stopPositionUpdateNotifier() {
        if (SHOW_LOGS)
            Log.v(TAG, "stopPositionUpdateNotifier, mPositionUpdateNotifier " + mPositionUpdateNotifier);
...
//            if (SHOW_LOGS) Log.v(TAG, "notifyPositionUpdated, mVideoStateListener " + mVideoStateListener);
