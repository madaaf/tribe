...
public class GlPreview extends GlTextureView implements Preview, Camera.PictureCallback {
  ...
  @Override public void setCameraHelper(final CameraHelper helper) {
    cameraHelper = helper;
  }
  ...
  @Override public void startPreview(final int measurePreviewWidth, final int measurePreviewHeight,
      final CameraStateListener listener) {
    synchronized (this) {
  ...
  private void startPreview() {
    synchronized (this) {
      ...
      if (measurePreviewWidth > 0 && measurePreviewHeight > 0) {
        cameraHelper.setupOptimalSizes(measurePreviewWidth, measurePreviewHeight,
            renderer.mMaxTextureSize);
      }
  ...
  @Override public void takePicture(final CaptureCallback callback) {
    takePicture(callback, true);
  }

  @Override public void takePicture(final CaptureCallback callback, final boolean autoFocus) {
    captureCallback = callback;
    cameraHelper.takePicture(this, autoFocus);
  }

  @Override public void startRecording(String fileId, AudioVisualizerCallback visualizerCallback) {
  ...
  private final class Renderer extends GlFrameBufferObjectRenderer
      implements PreviewTexture.OnFrameAvailableListener {
    ...
    @Override public void onDrawFrame(final GLES20FramebufferObject fbo) {
      synchronized (this) {
        if (mUpdateSurface) {
      ...
      if (mImageTexture != null) {
        ...
        try {
          if (recordShader != null && mFramebufferObject != null) {
            recordShader.setup();
            recordShader.setFrameSize(mFramebufferObject.getHeight(),
                mFramebufferObject.getWidth());
    ...
    @Override public synchronized void onFrameAvailable(final PreviewTexture previewTexture) {
      mUpdateSurface = true;
      requestRender();
    }
