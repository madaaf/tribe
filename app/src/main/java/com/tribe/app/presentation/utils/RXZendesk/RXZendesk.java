...
@Singleton public class RXZendesk {
  ...
  private Observable<List<MessageRealm>> messageListObservable;
  ...
  public Observable<List<MessageRealm>> getComments(String supportId) {
    if (messageListObservable == null) {
      messageListObservable =
          Observable.create((Subscriber<? super List<MessageRealm>> subscriber) -> {
  ...
  public void emitFriends(Subscriber subscriber) {
    List<MessageRealm> ok = new ArrayList<>();
    MessageRealm m = new MessageRealm();
    m.setData("SOOOEF ISJOIZ ");
    ok.add(m);
