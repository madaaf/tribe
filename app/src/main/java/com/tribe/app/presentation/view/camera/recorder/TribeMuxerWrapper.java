...
public class TribeMuxerWrapper {
  ...
  public void prepare(Context context, int width, int height) throws IOException {
    if (videoEncoder != null) videoEncoder.prepareEncoder(context, width, height);

    if (audioEncoder != null) audioEncoder.prepareEncoder(context, width, height);
  }

  public void startRecording() {
    if (videoEncoder != null) videoEncoder.startRecording();

    if (audioEncoder != null) audioEncoder.startRecording();
  }
  ...
  public void addEncoder(final MediaEncoder encoder) {
    if (encoder instanceof MediaVideoEncoder) {
      if (videoEncoder != null) throw new IllegalArgumentException("Video encoder already added.");

      videoEncoder = encoder;
    } else if (encoder instanceof MediaAudioEncoder) {
      if (audioEncoder != null) throw new IllegalArgumentException("Video encoder already added.");
  ...
  public synchronized int addTrack(final MediaFormat format) {
    if (isStarted) throw new IllegalStateException("muxer already started");

    final int trackIndex = mediaMuxer.addTrack(format);

    if (DEBUG) {
      Log.i(TAG,
          "addTrack:trackNum=" + encoderCount + ",trackId=" + trackIndex + ",format=" + format);
    }
  ...
  public synchronized void writeSampleData(final int trackIndex, final ByteBuffer byteBuf,
    ...
    if (startedCount > 0) mediaMuxer.writeSampleData(trackIndex, byteBuf, bufferInfo);
  }
}
