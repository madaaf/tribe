    ...
    public void onBindViewHolder(@NonNull List<FilterEntity> items, int position, @NonNull RecyclerView.ViewHolder holder) {
        ...
        if (filterEntity.isActivated()) {
            letterViewHolder.txtLetter.setOnClickListener(v -> {
                TransitionDrawable drawable = (TransitionDrawable) letterViewHolder.txtLetter.getBackground();
                drawable.startTransition(200);

                com.tribe.app.presentation.view.utils.AnimationUtils.animateTextColor(letterViewHolder.txtLetter, Color.BLACK, Color.WHITE, 200);

                Animation scaleLetterAnimation = AnimationUtils.loadAnimation(context, R.anim.scale_letters);
