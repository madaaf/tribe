...
public class RatingView extends FrameLayout {
  ...
  @BindViews({ R.id.txtReviewOne, R.id.txtReviewTwo }) List<TextViewFont> txtReviews;
  ...
  private void initUI() {
    ...
    subscriptions.add(Observable.timer(DURATION_SMALL, TimeUnit.MILLISECONDS)
  ...
  private void hideTxtRating(int index) {
    ...
    subscriptions.add(Observable.timer(initialDelay, TimeUnit.MILLISECONDS)
        .observeOn(AndroidSchedulers.mainThread())
        .subscribe(aLong -> {
          if (currentTxtReviewAnimation == txtReviews.size() - 1) {
            currentTxtReviewAnimation = 0;
          } else {
            currentTxtReviewAnimation++;
          }

          if (currentTxtReviewWording == wordings.length - 1) {
            currentTxtReviewWording = 0;
          } else {
            currentTxtReviewWording++;
          }
