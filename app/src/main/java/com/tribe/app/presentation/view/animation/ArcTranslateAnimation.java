package com.tribe.app.presentation.view.animation;
...
public class ArcTranslateAnimation extends Animation {
  ...
  private int fromXType = ABSOLUTE;
  private int toXType = ABSOLUTE;

  private int fromYType = ABSOLUTE;
  private int toYType = ABSOLUTE;
   ...
   * ABSOLUTE, or a percentage (where 1.0 is 100%) otherwise.
   */
  public ArcTranslateAnimation(int fromXType, float fromXValue, int toXType, float toXValue,
      int fromYType, float fromYValue, int toYType, float toYValue) {

    fromXValue = fromXValue;
    toXValue = toXValue;
    fromYValue = fromYValue;
    toYValue = toYValue;
  ...
  @Override public void initialize(int width, int height, int parentWidth, int parentHeight) {
    ...
    fromXDelta = resolveSize(fromXType, fromXValue, width, parentWidth);
    toXDelta = resolveSize(toXType, toXValue, width, parentWidth);
    fromYDelta = resolveSize(fromYType, fromYValue, height, parentHeight);
    toYDelta = resolveSize(toYType, toYValue, height, parentHeight);
  ...
  private long calcBezier(float interpolatedTime, float p0, float p1, float p2) {
    return Math.round((Math.pow((1 - interpolatedTime), 2) * p0) + (2
        * (1 - interpolatedTime)
        * interpolatedTime
        * p1) + (Math.pow(interpolatedTime, 2) * p2));
  }
}