...
public class MediaVideoEncoder extends MediaEncoder {
  ...
  @Override public void prepareEncoder(Context context, int width, int height) {
    ...
    try {
      MediaFormat format = MediaFormat.createVideoFormat(MIME_TYPE, endWidth, endHeight);
  ...
  @Override protected void signalEndOfInputStream() {
    if (DEBUG) Log.d(TAG, "sending EOS to encoder");
    try {
      if (isEncoding) mediaCodec.signalEndOfInputStream();
   ...
   */
  @SuppressWarnings("deprecation") protected static final MediaCodecInfo selectVideoCodec(
    ...
    for (int i = 0; i < numCodecs; i++) {
      ...
      if (!codecInfo.isEncoder() && !codecInfo.getName()
          .equals("OMX.google.h264.encoder")) {    // skipp decoder
        continue;
      }
