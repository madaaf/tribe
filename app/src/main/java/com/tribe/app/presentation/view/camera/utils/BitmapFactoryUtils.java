...
public class BitmapFactoryUtils {
  ...
  public static Bitmap decodeFile(final String filename, final int maxSize, final boolean square)
      throws IOException {
    ...
    if (square && bitmap.getWidth() != bitmap.getHeight()) {
      if (bitmap.getWidth() > bitmap.getHeight()) {
        final Bitmap _bitmap =
            Bitmap.createBitmap(bitmap, (bitmap.getWidth() - bitmap.getHeight()) / 2, 0,
                bitmap.getHeight(), bitmap.getHeight());
        bitmap.recycle();
        bitmap = _bitmap;
      } else if (bitmap.getWidth() < bitmap.getHeight()) {
        final Bitmap _bitmap =
            Bitmap.createBitmap(bitmap, 0, (bitmap.getHeight() - bitmap.getWidth()) / 2,
  ...
  public static Bitmap decodeStream(final InputStream is, final int startInSampleSize,
      final int add, final int multi) {
    final BitmapFactory.Options opts = new BitmapFactory.Options();
  ...
  public static Bitmap scale(Bitmap realImage, float maxImageSize, boolean filter) {
    float ratio = Math.max((float) maxImageSize / realImage.getWidth(),
  ...
  public static Bitmap decodeFile(final String pathName, final int startInSampleSize, final int add,
      final int multi) {
    final BitmapFactory.Options opts = new BitmapFactory.Options();
    int inSampleSize = startInSampleSize;
    while (true) {
  ...
  public static Bitmap decodeStream(final Context context, final String name,
