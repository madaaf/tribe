...
public class GroupActivity extends BaseActivity implements GroupMVPView {
  ...
  private void init(Bundle savedInstanceState) {
    ...
    txtAction.setOnClickListener(v -> {
      if (membershipId == null) {
        ...
        bundle.putString(TagManagerConstants.TEMPLATE,
            groupEntity.isCustom() ? groupEntity.getName() : TagManagerConstants.TEMPLATE_CUSTOM);
    ...
    if (savedInstanceState == null) {
      if (membershipId == null) {
        ...
        subscriptions.add(viewCreateGroup.onCreateNewGroup().subscribe(newGroup -> {
  ...
  private void setupAddMembersView(Serializable param) {
    ...
    subscriptions.add(viewAddMembersGroup.onClickMembers().subscribe(aVoid -> {
      setupMemberListGroupView();
    }));

    if (param instanceof Membership) {
      subscriptions.add(viewAddMembersGroup.onClickSettings().subscribe(aVoid -> {
        setupSettingsView();
      }));

      subscriptions.add(viewAddMembersGroup.onClickShareLink().subscribe(aVoid -> {
  ...
  private void setupSettingsView() {
    viewSettingsGroup =
        (SettingsGroupView) viewStack.pushWithParameter(R.layout.view_group_settings, membership);
    subscriptions.add(viewSettingsGroup.onEditGroup().subscribe(aVoid -> {
      setupUpdateView();
    }));

    subscriptions.add(viewSettingsGroup.onLeaveGroup().subscribe(aVoid -> {
      groupPresenter.leaveGroup(membershipId);
    }));

    subscriptions.add(viewSettingsGroup.onNotificationsChange().subscribe(aBoolean -> {
  ...
  private void computeTitle(boolean forward, View to) {
    if (to instanceof AddMembersGroupView) {
      setupTitle(groupEntity == null ? membership.getDisplayName()
          : getString(R.string.group_add_members_title), forward);
  ...
  private String getTitleForMembers() {
    return membership.getGroup().getMembers().size() + " " +
        (membership.getGroup().getMembers().size() > 1 ? getResources().getString(
            R.string.group_members) : getResources().getString(R.string.group_member));
  }
