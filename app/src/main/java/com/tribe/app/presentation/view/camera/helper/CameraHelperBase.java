...
public class CameraHelperBase
    implements CameraHelper, Camera.PictureCallback, Camera.AutoFocusCallback {
  ...
  @Override public final boolean isOpened() {
    return this.camera != null;
  }

  @Override public void nextCamera() {
    openCamera((cameraId + 1) % getNumberOfCameras());
  }

  @Override public int getNextCamera() {
    return (cameraId + 1) % getNumberOfCameras();
  }

  @Override public final void releaseCamera() {
    if (camera != null) {
      stopPreview();
      camera.release();
      camera = null;
  ...
  @Override public void setErrorCallback(final Camera.ErrorCallback cb) {
    camera.setErrorCallback(cb);
  }

  @Override public void setupOptimalSizes(int measureWidth, int measureHeight, int maxSize) {

  }

  @Override
  public void setPreviewTexture(final SurfaceTexture surfaceTexture, int width, int height)
      throws IOException {
  ...
  @Override public final void stopPreview() {
    synchronized (this) {
      if (camera != null) {
  ...
  @Override public void takePicture(Camera.PictureCallback callback) {
  ...
  @Override public final void onPictureTaken(final byte[] data, final Camera camera) {
  ...
  @Override public final void setPictureFormat(final int format) {
  ...
  @Override public final void cancelAutoFocus() {
  ...
  @Override public String getFlashMode() {
    return getCamera().getParameters().getFlashMode();
  }

  @Override public String getFocusMode() {
    return getCamera().getParameters().getFocusMode();
  }

  @Override public List<String> getSupportedFlashModes() {
    return getCamera().getParameters().getSupportedFlashModes();
  }

  @Override public List<String> getSupportedFocusModes() {
    return getCamera().getParameters().getSupportedFocusModes();
  }

  @Override public List<String> getSupportedFlashModes(final String... values) {
    return getContainsList(getCamera().getParameters().getSupportedFlashModes(), values);
  }

  @Override public List<String> getSupportedFocusModes(final String... values) {
  ...
  @Override public String switchFlashMode() {
    return switchFlashMode(getSupportedFlashModes());
  }

  @Override public String switchFlashMode(final String... values) {
  ...
  @Override public String switchFocusMode() {
    return switchFocusMode(getSupportedFocusModes());
  }

  @Override public String switchFocusMode(final String... values) {
  ...
  @Override public void setDisplayOrientation(final int degrees) {
    getCamera().setDisplayOrientation(degrees);
  }

  @Override public void setRotation(int degrees) {
  ...
  @Override public int getOptimalOrientation() {
    final WindowManager windowManager =
        (WindowManager) getContext().getSystemService(Context.WINDOW_SERVICE);
    final Display display = windowManager.getDefaultDisplay();
  ...
  @Override public int getPreviewOrientation() {
    final CameraHelper.CameraInfoCompat info = getCameraInfo();
    final int deviceOrientation = Degrees.getDisplayRotation(context);
    int displayOrientationVideo = Degrees.getDisplayOrientation(info.orientation, deviceOrientation,
    ...
    Log.d("CameraFragment",
        String.format("Orientations: Sensor = %d˚, Device = %d˚, Display = %d˚", info.orientation,
            deviceOrientation, displayOrientationVideo));
  ...
  @Override public void setPreviewCallback(final Camera.PreviewCallback cb) {
    ...
    if (cb != null) {
      try {
        ...
        buffer = new byte[Math.max(
            previewSize.getWidth() * previewSize.getHeight() * ImageFormat.getBitsPerPixel(
                parameters.getPreviewFormat()) / 8,
            pictureSize.getWidth() * pictureSize.getHeight() * ImageFormat.getBitsPerPixel(
                ImageFormat.RGB_565) / 8)];
  ...
  @Override public void onPreviewFrame(final Camera.PreviewCallback cb) {
    if (buffer != null) {
  ...
  @Override public CameraHelper.CameraInfoCompat getCameraInfo() {
    try {
  ...
  @Override public final boolean isFaceCamera() {
    return getCameraInfo().facing == CameraInfoCompat.CAMERA_FACING_FRONT;
  }

  @Override public void openCamera(final int cameraId) {
    releaseCamera();
  ...
  @Override public void initializeFocusMode() {
