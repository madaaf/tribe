...
public class CameraView extends ViewGroup implements TextureView.SurfaceTextureListener {
        ...
        if (child.equals(preview)) {
          ...
          if (cameraHelper.isOpened()) {
            ...
            if (previewSize != null) {
              ...
              final double scale = Math.min((double) childWidth / (double) previewWidth,
                  (double) childHeight / (double) previewHeight);
  ...
  public void switchCamera() {
    if (!isSwitching) {
      ...
      startPreview();
      subscriptions.add(Observable.timer(1000L, TimeUnit.MILLISECONDS)
