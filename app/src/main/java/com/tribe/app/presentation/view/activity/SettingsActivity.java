...
public class SettingsActivity extends BaseActivity implements SettingsMVPView {
  ...
  @Inject @ShareProfile Preference<Boolean> shareProfile;
  ...
  private void init(Bundle savedInstanceState) {
    ...
    txtAction.setOnClickListener(v -> {
      if (viewStack.getTopView() instanceof SettingsProfileView) {
        ...
        settingsPresenter.updateUser(viewSettingsProfile.getUsername(),
  ...
  private void setupSettingsView() {
    ...
    subscriptions.add(viewSettings.onShareProfileClick().subscribe(aVoid -> {
      ShareDialogProfileFragment shareDialogProfileFragment =
          ShareDialogProfileFragment.newInstance();
      shareDialogProfileFragment.show(getSupportFragmentManager(),
          ShareDialogProfileFragment.class.getName());
    ...
    subscriptions.add(viewSettings.onProfileClick().subscribe(aVoid -> {
      setupProfileView();
    }));

    subscriptions.add(viewSettings.onFiltersClick().subscribe(aVoid -> {
      setupFiltersView();
    }));

    subscriptions.add(viewSettings.onParametersClick().subscribe(aVoid -> {
      setupParametersView();
    }));

    subscriptions.add(viewSettings.onContactClick()
        .flatMap(aVoid -> DialogFactory.showBottomSheetForContact(this), ((aVoid, labelType) -> {
    ...
    subscriptions.add(viewSettings.onBlockedHiddenClick().subscribe(aVoid -> {
      setupBlockedHiddenView();
    }));

    subscriptions.add(viewSettings.onRateClick().subscribe(aVoid -> {
  ...
  private void setupFiltersView() {
    viewSettingsFilterTheme =
        (SettingsFilterThemeView) viewStack.push(R.layout.view_settings_filter_theme);
  }

  private void setupParametersView() {
    viewSettingsParameters =
        (SettingsParametersView) viewStack.push(R.layout.view_settings_parameters);
  ...
  private void setupBlockedHiddenView() {
    viewSettingsBlockedHidden =
        (SettingsBlockedHiddenView) viewStack.push(R.layout.view_settings_blocked_hidden);
    viewSettingsBlockedHidden.renderBlockedFriendshipList(blockedFriendshipList);

    subscriptions.add(viewSettingsBlockedHidden.onUpdateFriendship()
  ...
  @Override public void onFBContactsSync(int count) {

  }

  @Override public void onAddressBookContactSync(int count) {

  }

  @Override public void onSuccessSync() {

  }

  @Override public void friendshipUpdated(Friendship friendship) {
