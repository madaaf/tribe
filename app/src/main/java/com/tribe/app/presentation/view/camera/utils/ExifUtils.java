...
public class ExifUtils {
  ...
  public static void save(final String filename, final Date datetime, final int orientation,
      final Boolean flash, final Location location) throws IOException {
    final ExifInterface exif = new ExifInterface(filename);

    if (datetime != null) {
      exif.setAttribute(ExifInterface.TAG_DATETIME,
          new SimpleDateFormat(DATETIME_FORMAT, Locale.ENGLISH).format(datetime));
    }
    ...
    if (location != null) {
      ...
      exif.setAttribute(TAG_GPS_DATESTAMP,
          new SimpleDateFormat(DATE_FORMAT, Locale.ENGLISH).format(datetime));
    }
  ...
  private static String formatExifGpsDMS(final double d) {
    ...
    return new StringBuilder().append(_degrees)
        .append("/1,")
        .append(_minutes)
        .append("/1,")
        .append(_seconds.substring(0, Math.min(_seconds.length(), 4)))
        .append("/1000")
        .toString();
  }
