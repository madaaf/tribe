...
public class LiveRoomView2 extends ViewGroup {
  ...
  public LiveRoomView2(Context context) {
    super(context);
    type = DEFAULT_TYPE;
  }

  public LiveRoomView2(Context context, AttributeSet attrs) {
    super(context, attrs);
    type = DEFAULT_TYPE;
  }
  ...
  private int measureOneOrTwoChildren(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    combineMeasuredStates(childState,
        measureTribeChild(getChildAt(0), widthMeasureSpec, heightMeasureSpec, fullWidth,
            partialHeight));

    if (getChildCount() == 2) {
      combineMeasuredStates(childState,
          measureTribeChild(getChildAt(1), widthMeasureSpec, heightMeasureSpec, fullWidth,
              partialHeight));
    }
  ...
  private int measureThreeOrFourChildren(int widthMeasureSpec, int heightMeasureSpec) {
    return measureThreeOrMoreChildren(widthMeasureSpec, heightMeasureSpec,
        getChildCount() == 3 ? 0 : -1);
  }
  ...
  private int measureThreeOrMoreChildren(int widthMeasureSpec, int heightMeasureSpec,
      int fullIndex) {
    int childState = 0;
  ...
  private int measureTribeChild(View child, int widthMeasureSpec, int heightMeasureSpec,
    ...
    if (child.getVisibility() != GONE) {
      child.measure(
          getChildMeasureSpec(widthMeasureSpec, getPaddingLeft() + getPaddingRight(), desiredWidth),
  ...
  private int measureTribeChildrenLinear(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    for (int i = 0; i < getChildCount(); i++) {
      ...
      if (child.getVisibility() != GONE) {
        child.measure(
            getChildMeasureSpec(widthMeasureSpec, getPaddingLeft() + getPaddingRight(), fullWidth),
            getChildMeasureSpec(heightMeasureSpec, getPaddingTop() + getPaddingBottom(),
                partialHeight));
      }
  ...
  private void layoutChildrenGrid() {
    for (int i = 0; i < getChildCount(); i++) {
      ...
      if (i == 2 || i == 3 || i == 5 || i == 7) {
        childLeft = (getPaddingLeft() + partialWidth);
      } else {
        childLeft = getPaddingLeft();
      }

      if (i == 0 || i == 3) {
        childTop = (getPaddingTop() + partialHeight);
      } else if (i == 4 || i == 5) {
        childTop = (getPaddingTop() + partialHeight * 2);
      } else if (i == 6 || i == 7) {
        childTop = (getPaddingTop() + partialHeight * 3);
      } else {
  ...
  private void layoutChildrenLinear() {
    for (int i = 0; i < getChildCount(); i++) {
      ...
      if (i == 0) {
        childTop = (getPaddingTop() + (getChildCount() >= 2 ? partialHeight : 0));
      } else if (i == 1) {
        childTop = (getPaddingTop());
      } else {
        childTop = (getPaddingTop() + partialHeight * i);
      }
      child.layout(childLeft, childTop, childLeft + child.getMeasuredWidth(),
          childTop + child.getMeasuredHeight());
    }
