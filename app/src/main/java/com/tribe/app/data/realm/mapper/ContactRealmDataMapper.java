 ...
 */
@Singleton public class ContactRealmDataMapper {

  private final UserRealmDataMapper userRealmDataMapper;
  private final PhoneRealmDataMapper phoneRealmDataMapper;

  @Inject public ContactRealmDataMapper(UserRealmDataMapper userRealmDataMapper,
      PhoneRealmDataMapper phoneRealmDataMapper) {
    this.userRealmDataMapper = userRealmDataMapper;
    this.phoneRealmDataMapper = phoneRealmDataMapper;
  }

  /**
   * Transform a {@link com.tribe.app.data.realm.ContactInterface} into an {@link
   * com.tribe.app.domain.entity.Contact}.
   *
   * @param contactI Object to be transformed.
   * @return {@link com.tribe.app.domain.entity.Contact} if valid {@link ContactInterface} otherwise
   * null.
   */
  public Contact transform(ContactInterface contactI) {
    if (contactI instanceof ContactABRealm) {
      ...
      contact.setUserList(userRealmDataMapper.transform(contactABRealm.getUsers(), true));
      contact.setHowManyFriends(contactABRealm.getHowManyFriends());
      contact.setcommonFriendsNameList(contactABRealm.getcommonFriendsNameList());
      contact.setNew(contactABRealm.isNew());
      return contact;
    } else {
      ...
      contact.setUserList(userRealmDataMapper.transform(contactFBRealm.getUserList(), true));
      contact.setHowManyFriends(contactFBRealm.getHowManyFriends());
      contact.setcommonFriendsNameList(contactFBRealm.getcommonFriendsNameList());
      contact.setNew(contactFBRealm.isNew());
      return contact;
    }
  }
