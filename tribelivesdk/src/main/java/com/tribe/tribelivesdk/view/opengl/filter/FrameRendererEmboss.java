...
public class FrameRendererEmboss extends FrameRendererDrawOrigin {
  ...
  private static final String fragmentShaderEmboss = "" +
  ...
  @Override public boolean init(boolean isExternalOES) {
    if (setProgramDefualt(getVertexShaderString(), getFragmentShaderString(), isExternalOES)) {
      program.bind();
      program.sendUniformf(SAMPLER_STEPS, 1.0f / 640.0f, 1.0f / 640.0f);
      return true;
    }
    return false;
  }
  ...
  @Override public void setTextureSize(int w, int h) {
    ...
    program.bind();
    program.sendUniformf(SAMPLER_STEPS, 1.0f / w, 1.0f / h);
  }

  @Override public String getFragmentShaderString() {
    return fragmentShaderEmboss;
  }
}
