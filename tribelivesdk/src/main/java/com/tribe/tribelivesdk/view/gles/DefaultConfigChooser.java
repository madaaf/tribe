package com.tribe.tribelivesdk.view.gles;
...
public class DefaultConfigChooser implements GLSurfaceView.EGLConfigChooser {
  ...
  @NonNull private final int[] mConfigSpec;
  ...
  public DefaultConfigChooser(final boolean withDepthBuffer, final int version) {
    this(USE_RGB_888 ? 8 : 5, USE_RGB_888 ? 8 : 6, USE_RGB_888 ? 8 : 5, 0, withDepthBuffer ? 16 : 0,
        0, version);
  }

  public DefaultConfigChooser(final int redSize, final int greenSize, final int blueSize,
      final int alphaSize, final int depthSize, final int stencilSize, final int version) {
    mConfigSpec = filterConfigSpec(new int[] {
        EGL_RED_SIZE, redSize, EGL_GREEN_SIZE, greenSize, EGL_BLUE_SIZE, blueSize, EGL_ALPHA_SIZE,
        alphaSize, EGL_DEPTH_SIZE, depthSize, EGL_STENCIL_SIZE, stencilSize, EGL_NONE
    }, version);
    mRedSize = redSize;
  ...
  @NonNull private int[] filterConfigSpec(@NonNull final int[] configSpec, final int version) {
    if (version != 2) {
      return configSpec;
    }
    /*
  ...
  @Nullable EGLConfig chooseConfig(@NonNull final EGL10 egl, final EGLDisplay display,
      @NonNull final EGLConfig[] configs) {
  ...
  private int findConfigAttribute(@NonNull final EGL10 egl, final EGLDisplay display,
