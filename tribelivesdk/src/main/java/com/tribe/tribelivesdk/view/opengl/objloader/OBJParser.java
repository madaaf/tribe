...
public class OBJParser {
  ...
  public TDModelPart parseOBJ(InputStream inputStream) {
    ...
    if (faces != null) {//if not this is not the start of the first group
      part = new TDModelPart(faces, vtPointer, vnPointer, m, vn);
      part.setIVT(faces, v, vt);
    }
  ...
  private void processFLine(String line) {
    ...
    if (tokens[1].matches("[0-9]+")) {//f: v
      if (c == 4) {//3 faces
        ...
        faces.addAll(Triangulator.triangulate(
            polygon)); //triangulate the polygon and add the resulting faces
      }
    }
