...
public class ProgramObject {
  ...
  public boolean init(final String vsh, final String fsh, int programID) {
    if (programID == 0) programID = GLES20.glCreateProgram();

    assert programID != 0 : "glCreateProgram failed!";

    if (vertexShader != null) vertexShader.release();
    if (fragmentShader != null) fragmentShader.release();

    vertexShader = new ShaderObject(vsh, GLES20.GL_VERTEX_SHADER);
    fragmentShader = new ShaderObject(fsh, GLES20.GL_FRAGMENT_SHADER);
  ...
  public int getUniformLoc(final String name) {
    int uniform = GLES20.glGetUniformLocation(programId, name);
    if (Common.DEBUG) {
      if (uniform < 0) Log.e(LOG_TAG, String.format("uniform name %s does not exist", name));
    }
    return uniform;
  }
  ...
  public static class ShaderObject {
    ...
    public boolean init(final String shaderCode, final int shaderType) {
      ...
      shaderID = loadShader(shaderType, shaderCode);

      //Debug Only
      assert shaderID != 0 : "Shader Create Failed!";
