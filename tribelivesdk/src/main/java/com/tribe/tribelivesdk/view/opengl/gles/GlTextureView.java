...
import com.tribe.tribelivesdk.view.ScalableTextureView;
 ...
 */
public class GlTextureView extends ScalableTextureView
  ...
  @Override public void setSurfaceTextureListener(final SurfaceTextureListener listener) {
  ...
  public void setEGLConfigChooser(final int redSize, final int greenSize, final int blueSize,
    ...
    setEGLConfigChooser(
        new DefaultConfigChooser(redSize, greenSize, blueSize, alphaSize, depthSize, stencilSize,
  ...
  private static class EglHelper {
    ...
    public boolean createSurface() {
      ...
      if (view != null) {
        mEglSurface =
            view.mEGLWindowSurfaceFactory.createWindowSurface(mEgl, mEglDisplay, mEglConfig,
    ...
    public static void logEglErrorAsWarning(final String tag, final String function,
        final int error) {
      Log.w(tag, formatEglError(function, error));
    }

    @NonNull public static String formatEglError(final String function, final int error) {
      return function + " failed: " + EGLLogWrapper.getErrorString(error);
    }

    private final WeakReference<GlTextureView> mGLSurfaceViewWeakRef;
    EGL10 mEgl;
    @Nullable EGLDisplay mEglDisplay;
    @Nullable EGLSurface mEglSurface;
    @Nullable EGLConfig mEglConfig;
    @Nullable EGLContext mEglContext;
  }
          ...
          synchronized (sGLThreadManager) {
            while (true) {
              ...
              if (pausing && mHaveEglContext) {
                final GlTextureView view = mGLSurfaceViewWeakRef.get();
                final boolean preserveEglContextOnPause =
                    view != null && view.mPreserveEGLContextOnPause;
  ...
  private static class GLThreadManager {
    ...
    public synchronized void checkGLDriver(@NonNull final GL10 gl) {
      if (!mGLESDriverCheckComplete) {
        ...
        mLimitedGLESContexts = !mMultipleGLESContextsAllowed ||
            (Build.VERSION.SDK_INT < JELLY_BEAN && renderer.startsWith(kADRENO));
        if (LOG_SURFACE) {
          Log.w(TAG, "checkGLDriver renderer = \"" +
              renderer +
    ...
    @Nullable private GLThread mEglOwner;
  }
