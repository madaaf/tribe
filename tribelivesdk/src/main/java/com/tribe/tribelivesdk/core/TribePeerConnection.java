...
public class TribePeerConnection {
    ...
    private void init(PeerConnectionFactory peerConnectionFactory, boolean isOffer) {
        ...
        peerConnection = peerConnectionFactory.createPeerConnection(iceServerList, sdpObserver.constraints, peerConnectionObserver);
        sdpObserver.setPeerConnection(peerConnection);

        subscriptions.add(
                peerConnectionObserver.onShouldCreateOffer()
                        .subscribe(aVoid -> sdpObserver.createOffer())
        );
        ...
        subscriptions.add(
                peerConnectionObserver.onReceivedMediaStream()
    ...
    public Observable<TribeAnswer> onReadyToSendSdpAnswer() {
        return sdpObserver.onReadyToSendSdpAnswer()
                .map(sessionDescription -> {
