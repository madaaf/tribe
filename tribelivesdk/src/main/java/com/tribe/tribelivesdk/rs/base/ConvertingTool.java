...
import com.tribe.tribelivesdk.rs.Nv21Image;
...
public class ConvertingTool<T> {
  ...
  public Bitmap doComputation(RenderScript rs, Bitmap inputBitmap, T scriptParams) {
    ...
    Bitmap outputBitmap =
        Bitmap.createBitmap(inputBitmap.getWidth(), inputBitmap.getHeight(), config);
    return doComputation(rs, inputBitmap, outputBitmap, scriptParams);
  }

  private Bitmap doComputation(RenderScript rs, Bitmap inputBitmap, Bitmap outputBitmap,
      T scriptParams) {
  ...
  //  Allocation aout = Allocation.createFromBitmap(rs, outputBitmap);
  //
  //  RSToolboxContext rsToolboxContext =
  //      RSToolboxContext.createFromByteArray(rs, nv21ByteArray, width, height);
  //
  //  this.tool.runScript(rsToolboxContext, aout, scriptParams);
  //
  //  Nv21Image nv21Output = Nv21Image.bitmapToNV21(rs, outputBitmap);
