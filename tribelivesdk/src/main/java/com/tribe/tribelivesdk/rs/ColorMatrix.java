...
public class ColorMatrix {
  ...
  private static ConvertingTool.BaseToolScript colorMatrixToolScript =
      new ConvertingTool.BaseToolScript<ColorMatrixParams>() {
        @Override public void runScript(RSToolboxContext rsToolboxContext, Allocation aout,
            ColorMatrixParams scriptParams) {
          ScriptIntrinsicColorMatrix colorMatrixScript =
              ScriptIntrinsicColorMatrix.create(rsToolboxContext.rs,
  ...
  private static Bitmap applyMatrix(RenderScript rs, Bitmap inputBitmap, Matrix3f matrix3f,
      Optional<Float4> addTerms) {
    ColorMatrixParams matrixParam = ColorMatrixParams.createWithMatrix(matrix3f, addTerms);
    return doColorMatrixComputation(rs, inputBitmap, matrixParam);
  }

  private static Bitmap applyMatrix(RenderScript rs, Bitmap inputBitmap, Matrix4f matrix4f,
      Optional<Float4> addTerms) {
    ColorMatrixParams matrixParam = ColorMatrixParams.createWithMatrix(matrix4f, addTerms);
    return doColorMatrixComputation(rs, inputBitmap, matrixParam);
  }
