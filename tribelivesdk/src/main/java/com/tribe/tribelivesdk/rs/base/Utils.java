...
import com.tribe.tribelivesdk.rs.Histogram;
...
public class Utils {
  ...
  public static Bitmap drawHistograms(int[] histograms, int channels) {
    Bitmap outputBitmap =
        Bitmap.createBitmap(Histogram.COLOR_DEPTH * channels, Histogram.COLOR_DEPTH,
    ...
    for (int y = 0; y < Histogram.COLOR_DEPTH; y++) {
      for (int x = 0; x < Histogram.COLOR_DEPTH * channels; x++) {
        ...
        int height =
            (int) ((histograms[c + i * channels] / maxes[c]) * (Histogram.COLOR_DEPTH - 1.f));
